...
Flow matching is a generative modeling technique that learns a vector field to transform simple
noise into complex data. This page builds up the framework step by step, from time-dependent vector
fields and conditional probability paths to the loss function that's used to train a model. Along
the way, there are interactive visualizations and a model you can train in the browser.
...
Formally, such a field is defined as
...
$x_{\mathrm{0.123}}$, $u_{\mathrm{0.456}}$.
...
In particular, we can construct a Gaussian conditional probability path where we start with the
standard Gaussian distribution and then successively move the mean towards $z$ and the covariance
matrix towards zero. This is governed by "noise schedulers" $\alpha_t$ and $\beta_t$:
...
Let's use an in-browser model that can approximate the moons toy dataset:

[[ moons-dataset-visualization ]]

The model has already been trained, the loss curve is shown below. If you want, you can train a
model from scratch by clicking on "Train":
...
[[ flow-matching-inference-visualization ]]
